...
async fn main() -> Result<(), String> {
    ...
    if !config_dir.exists() {
        ...
        create_dir_all(&config_dir)
            .await
            .expect(format!("could not create config dir {:?}", &config_dir).as_str());
    }
    ...
    if !data_dir.exists() {
        ...
        create_dir_all(&data_dir)
            .await
            .expect(format!("could not create data dir {:?}", &data_dir).as_str());
    }
    ...
    let db: Surreal<Db> = Surreal::new::<RocksDb>(db_dir)
        .await
        .expect("Failed to create surreal db handle");
            ...
            match missions {
                Ok(missions) => {
                    ...
                    missions.iter().for_each(|s| {
                        ...
                        println!(
                            "Target System: {}, Target Faction: {}, Faction: {}, Count {}",
                            target_system, target_faction, faction, count
                        );
                    });
                    println!("------------------------------------------------------");
                }
                Err(e) => {
                    error!("Failed to get missions: {}", e);
                }
            }
